...
namespace graphicbox2d
{
    ...
    internal static class GraphicCaluculate_Old
    {
        ...
        public static bool IsCrossArcAndCircle(PointF Circle1Center, float R1, float StartAngle, float EndAngle, PointF Circle2Center, float R2, out eIsCrossArcAndCircle Ret)
        {
            ...
            // 度数法に変換
            float towAngle = GraphicCaluculate_Old.RadianToDegree(towRadian);
            ...
            else
            {
                ...
                PointF ArcStartPt = new PointF(
                    Circle1Center.X + R1 * (float)Math.Cos(GraphicCaluculate_Old.DegreeToRadian(StartAngle)),
                    Circle1Center.Y + R1 * (float)Math.Sin(GraphicCaluculate_Old.DegreeToRadian(StartAngle))
                    );
                ...
                PointF ArcEndPt = new PointF(
                    Circle1Center.X + R1 * (float)Math.Cos(GraphicCaluculate_Old.DegreeToRadian(EndAngle)),
                    Circle1Center.Y + R1 * (float)Math.Sin(GraphicCaluculate_Old.DegreeToRadian(EndAngle))
                    );
        ...
        public static PointF [] ConvertGridPointToClientPoint(PointF[] GridPoints)
        {
            PointF[] ClientPoints = new PointF[GridPoints.Length];
        ...
        public static PointF[] GetBoundingBoxLine(PointF Start, PointF End, int Width, eCalculateType calculateType = eCalculateType.Client)
        {
            ...
            Vector2 line = End.ToVector2() - Start.ToVector2();
            ...
            {
                length = GraphicCaluculate_Old.ConvertClientLengthToGridLength(Width + LINE_BOUNDING_BOX_WITDH);
            }
            else
            {
            ...
            boundingBox[0] = (Start.ToVector2() + TyokkouVector).ToPointF();
            boundingBox[1] = (End.ToVector2() + TyokkouVector).ToPointF();
            boundingBox[2] = (End.ToVector2() - TyokkouVector).ToPointF();
            boundingBox[3] = (Start.ToVector2() - TyokkouVector).ToPointF();
        ...
        {
            PointF[] boundingBox = new PointF[4];
            boundingBox[0] = new PointF(X - R, Y - R);
            boundingBox[1] = new PointF(X + R, Y - R);
            boundingBox[2] = new PointF(X + R, Y + R);
            boundingBox[3] = new PointF(X - R, Y + R);
        ...
        /// <param name="DrawFontSize">フォントサイズ</param>
        /// <param name="FontName">フォント名</param>
        /// <param name="Angle">回転角度</param>
        /// <param name="calculateType">座標系の種類（Client または Grid）</param>
        /// <returns>バウンディングボックスの4頂点座標</returns>

        public static PointF[] GetBoundingBoxText(float X, float Y, string Text, float DrawFontSize, string FontName, float Angle, eCalculateType calculateType = eCalculateType.Client)
        {
            Font font = DrawManager_Old.GetFont(FontName, DrawFontSize);

            PointF[] BoundingBoxPoints = new PointF[4];
            RectangleF TextRect = GraphicCaluculate_Old.GetTextRectangle(Text, font);

            if (calculateType == eCalculateType.Grid)
            {
                TextRect.Height = -TextRect.Height;

                TextRect.Width  = ConvertClientLengthToGridLength((int)TextRect.Width);
                TextRect.Height = ConvertClientLengthToGridLength((int)TextRect.Height);
            ...
            // テキスト矩形の4頂点を取得
            BoundingBoxPoints[0] = new PointF(0, 0);
            BoundingBoxPoints[1] = new PointF(TextRect.Width, 0);
            BoundingBoxPoints[2] = new PointF(TextRect.Width, TextRect.Height);
            BoundingBoxPoints[3] = new PointF(0, TextRect.Height);

            // テキスト矩形を回転させる
            if (Comp.IsEqual(Angle, 0f) == false)
            {
